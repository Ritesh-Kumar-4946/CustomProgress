...
public class UnacademyLoader extends View {
     ...
     * Listener.
     */
    private OnUnacademyLoaderChangeListener mOnUnacademyLoaderChangeListener;
    ...
    private void initAttributes(TypedArray attrArray) {
        mCircleXRadius = attrArray.getDimension(R.styleable.ua_UnacademyLoader_ua_circle_x_radius, DEFAULT_CIRCLE_X_RADIUS);
        mCircleYRadius = attrArray.getDimension(R.styleable.ua_UnacademyLoader_ua_circle_y_radius, DEFAULT_CIRCLE_Y_RADIUS);
        mPointerStrokeWidth = attrArray.getDimension(R.styleable.ua_UnacademyLoader_ua_pointer_stroke_width, DEFAULT_POINTER_STROKE_WIDTH);
        mPointerHaloWidth = attrArray.getDimension(R.styleable.ua_UnacademyLoader_ua_pointer_halo_width, DEFAULT_POINTER_HALO_WIDTH);
        mPointerHaloBorderWidth = attrArray.getDimension(R.styleable.ua_UnacademyLoader_ua_pointer_halo_border_width, DEFAULT_POINTER_HALO_BORDER_WIDTH);
        mCircleStrokeWidth = attrArray.getDimension(R.styleable.ua_UnacademyLoader_ua_circle_stroke_width, DEFAULT_CIRCLE_STROKE_WIDTH);

        int circleStyle = attrArray.getInt(R.styleable.ua_UnacademyLoader_ua_circle_style, DEFAULT_CIRCLE_STYLE);
        mCircleStyle = Paint.Cap.values()[circleStyle];

        mPointerColor = attrArray.getColor(R.styleable.ua_UnacademyLoader_ua_pointer_color, DEFAULT_POINTER_COLOR);
        mPointerHaloColor = attrArray.getColor(R.styleable.ua_UnacademyLoader_ua_pointer_halo_color, DEFAULT_POINTER_HALO_COLOR);
        mPointerHaloColorOnTouch = attrArray.getColor(R.styleable.ua_UnacademyLoader_ua_pointer_halo_color_ontouch, DEFAULT_POINTER_HALO_COLOR_ONTOUCH);
        mCircleColor = attrArray.getColor(R.styleable.ua_UnacademyLoader_ua_circle_color, DEFAULT_CIRCLE_COLOR);
        mCircleProgressColor = attrArray.getColor(R.styleable.ua_UnacademyLoader_ua_circle_progress_color, DEFAULT_CIRCLE_PROGRESS_COLOR);
        mCircleFillColor = attrArray.getColor(R.styleable.ua_UnacademyLoader_ua_circle_fill, DEFAULT_CIRCLE_FILL_COLOR);

        mPointerAlpha = Color.alpha(mPointerHaloColor);

        mPointerAlphaOnTouch = attrArray.getInt(R.styleable.ua_UnacademyLoader_ua_pointer_alpha_ontouch, DEFAULT_POINTER_ALPHA_ONTOUCH);
        if (mPointerAlphaOnTouch > 255 || mPointerAlphaOnTouch < 0) {
            mPointerAlphaOnTouch = DEFAULT_POINTER_ALPHA_ONTOUCH;
        }

        mMax = attrArray.getInt(R.styleable.ua_UnacademyLoader_ua_max, DEFAULT_MAX);
        mProgress = attrArray.getInt(R.styleable.ua_UnacademyLoader_ua_progress, DEFAULT_PROGRESS);
        mCustomRadii = attrArray.getBoolean(R.styleable.ua_UnacademyLoader_ua_use_custom_radii, DEFAULT_USE_CUSTOM_RADII);
        mMaintainEqualCircle = attrArray.getBoolean(R.styleable.ua_UnacademyLoader_ua_maintain_equal_circle, DEFAULT_MAINTAIN_EQUAL_CIRCLE);
        mMoveOutsideCircle = attrArray.getBoolean(R.styleable.ua_UnacademyLoader_ua_move_outside_circle, DEFAULT_MOVE_OUTSIDE_CIRCLE);
        mLockEnabled = attrArray.getBoolean(R.styleable.ua_UnacademyLoader_ua_lock_enabled, DEFAULT_LOCK_ENABLED);
        mDisablePointer = attrArray.getBoolean(R.styleable.ua_UnacademyLoader_ua_disable_pointer, DEFAULT_DISABLE_POINTER);
        mNegativeEnabled = attrArray.getBoolean(R.styleable.ua_UnacademyLoader_ua_negative_enabled, DEFAULT_NEGATIVE_ENABLED);
        mIsInNegativeHalf = false;
        mDisableProgressGlow = attrArray.getBoolean(R.styleable.ua_UnacademyLoader_ua_disable_progress_glow, DEFAULT_DISABLE_PROGRESS_GLOW);
        mHideProgressWhenEmpty = attrArray.getBoolean(R.styleable.ua_UnacademyLoader_ua_hide_progress_when_empty, DEFAULT_CS_HIDE_PROGRESS_WHEN_EMPTY);

        // Modulo 360 right now to avoid constant conversion
        mStartAngle = ((360f + (attrArray.getFloat((R.styleable.ua_UnacademyLoader_ua_start_angle), DEFAULT_START_ANGLE) % 360f)) % 360f);
        mEndAngle = ((360f + (attrArray.getFloat((R.styleable.ua_UnacademyLoader_ua_end_angle), DEFAULT_END_ANGLE) % 360f)) % 360f);
        ...
        // Modulo 360 right now to avoid constant conversion
        mPointerAngle = ((360f + (attrArray.getFloat((R.styleable.ua_UnacademyLoader_ua_pointer_angle), DEFAULT_POINTER_ANGLE) % 360f)) % 360f);
        if (mPointerAngle == 0f) {
...
//            }
    ...
    public void setProgress(float progress) {
        if (mProgress != progress) {
            ...
            if (mOnUnacademyLoaderChangeListener != null) {
                mOnUnacademyLoaderChangeListener.onProgressChanged(this, progress, false);
            }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (((touchEventRadius >= innerRadius) && (touchEventRadius <= outerRadius)) &&
                    ...
                    invalidate();
                    if (mOnUnacademyLoaderChangeListener != null) {
                        mOnUnacademyLoaderChangeListener.onStartTrackingTouch(this);
                    }
                    ...
                    if (mOnUnacademyLoaderChangeListener != null) {
                        mOnUnacademyLoaderChangeListener.onStartTrackingTouch(this);
                        mOnUnacademyLoaderChangeListener.onProgressChanged(this, getProgress(), true);
                    }
                    mUserIsMovingPointer = true;
                    mLockAtEnd = false;
                    mLockAtStart = false;
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mUserIsMovingPointer) {
                    ...
                    invalidate();
                    if (mOnUnacademyLoaderChangeListener != null) {
                        mOnUnacademyLoaderChangeListener.onStopTrackingTouch(this);
                    }
                } else {
                    return false;
                }
    ...
    private void init(AttributeSet attrs, int defStyle) {
        final TypedArray attrArray = getContext().obtainStyledAttributes(attrs, R.styleable.ua_UnacademyLoader, defStyle, 0);
    ...
    public UnacademyLoader(Context context) {
        super(context);
        init(null, 0);
    }

    public UnacademyLoader(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public UnacademyLoader(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setOnSeekBarChangeListener(OnUnacademyLoaderChangeListener l) {
        mOnUnacademyLoaderChangeListener = l;
    }

    /**
     * Listener for the CircularSeekBar. Implements the same methods as the normal OnSeekBarChangeListener.
     */
    public interface OnUnacademyLoaderChangeListener {

        public abstract void onProgressChanged(UnacademyLoader circularSeekBar, float progress, boolean fromUser);

        public abstract void onStopTrackingTouch(UnacademyLoader seekBar);

        public abstract void onStartTrackingTouch(UnacademyLoader seekBar);
    }
    ...
    public void setMax(float max) {
        if (max > 0) { // Check to make sure it's greater than zero
            if (max <= mProgress) {
                ...
                if (mOnUnacademyLoaderChangeListener != null) {
                    mOnUnacademyLoaderChangeListener.onProgressChanged(this, mIsInNegativeHalf ? -mProgress : mProgress, false);
                }
            }
            mMax = max;
